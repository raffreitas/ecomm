...
using Ecomm.Orders.Application.Orders.MarkOrderAsFailed;
...
internal class PaymentRejectedConsumer : BackgroundService
{
    private const string CustomerCreatedQueueName = "payment.rejected";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.ReceivedAsync += async (sender, eventArgs) =>
        {
            try
            {
                ...
                var command = JsonSerializer.Deserialize<MarkOrderAsFailedCommand>(createCustomerJson);
    ...
    private async Task SendCommand(MarkOrderAsFailedCommand command, CancellationToken cancellationToken)
    {
